...
# Multiple sequence alignment between the coding sequence of ALG1 (partitioned by exon) and its 8 known pseudogenes 
...
outfile <- file.path(workdir, "manuscript/Main_Figures/Figure_7/Figure_7b.pdf")
...
tx.info <- read.table(file.path(workdir, "manuscript/Main_Figures/Figure_7/ALG1/sequences.fa"), sep = "\t", 
    header = FALSE) %>% filter(row_number() %% 2 == 1)
inDF <- bind_cols(data.frame(do.call(rbind, strsplit(read.table(file.path(workdir, "manuscript/Main_Figures/Figure_7/ALG1/sequences.fa"), 
