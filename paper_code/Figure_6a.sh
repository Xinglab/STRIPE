...
# Sashimi plot visualization of haplotype-resolved MTFMT splicing patterns in individual Q1507.

# We used the following Python code to phase reads for MTFMT. Analysis of unphased TEQUILA-seq reads for individual
# Q1507 uncovered two splicing defects: exon 4 skipping and inclusion of a 53 bp pseudoexon between exons 6 and 7.
...
# patient_id, gene_panel, gene_name = 'Q1507', 'PMD-359', 'MTFMT'
...
#     '-' + str(target[2]) + ' > ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam && ' + 
#     samtools + ' index ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam')
# samfile = pysam.AlignmentFile(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam', 'rb')
...
# qscore = PhasingQuality(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam', assignments, target[1], target[2])
# with open(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/phasing_stats.txt', 'w') as quality_file:
...
# WriteBam(samtools, {key for key in assignments if assignments[key] == 'hap1'}, workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam', 
#     workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/hap1_reads.bam')

# WriteBam(samtools, {key for key in assignments if assignments[key] == 'hap2'}, workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam', 
#     workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/hap2_reads.bam')

# WriteBam(samtools, {key for key in assignments if assignments[key] == 'NA'}, workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam', 
#     workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/unassigned_reads.bam')

# _ = os.system('rm ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam && rm ' + 
#     workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/all_reads.bam.bai')
...
# Establish working directories and file paths
WORKDIR="/mnt/isilon/lin_lab_share/STRIPE"
OUTFILE="$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/Figure_6a.pdf"
mkdir -p "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp"
...
awk '{if($5 == "MTFMT"){print $4;}}' "$WORKDIR/PMD/references/target_genes.bed" | \
    ...
    > "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input.gtf"

# Construct text file containing file paths to haplotype-resolved BAM files
echo -e "Haplotype 1\t$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/hap1_reads.bam\t1" \
    > "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input_bam.txt"
echo -e "Haplotype 2\t$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/hap2_reads.bam\t2" \
    >> "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input_bam.txt"

# Use ggsashimi.py to visualize haplotype-resolved MTFMT splicing patterns in individual Q1507
# Only visualize splice junctions supported by at least 100 reads
(singularity run -B "$WORKDIR" docker://guigolab/ggsashimi:v1.1.5 -b "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input_bam.txt" \
    -c "chr15:65003056-65029639" -o "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/Figure_6a" -M 100 \
    -g "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input.gtf" --shrink -C 3 --alpha 1 --height 1 \
    --ann-height 0.5 --width 3.5 --base-size 6) > /dev/null 2>&1

rm -rf "$WORKDIR/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp"
