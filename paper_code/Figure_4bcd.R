...
# (b) Full-length structures and (c) isoform-level proportions of PIGQ transcripts detected from TEQUILA-seq data of 
# individual CDG-P09 and other cohort individuals. Shaded and unshaded regions within transcript structures represent 
# putative coding sequences and untranslated regions respectively. Transcripts featuring exon 4 skipping or usage of 
# a polyadenylation site within intron 4 with the highest isoform-level proportion in Haplotype 1 or transcripts with an 
# average isoform-level proportion ≥ 10% across other cohort individuals are displayed in (b, c). (d) Read counts for 
# PIGQ transcripts displayed in (b, c) in individual CDG-P09 (resolved by haplotype).
...
target.gene <- read.table(file.path(workdir, "CDG/references/target_genes.bed"), sep = "\t", header = FALSE) %>% filter(V5 == "PIGQ")
gencode.gtf <- "/scr1/users/wangr5/references/gencode.v45.annotation.gtf"

# Create a temporary directory
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/gene.gtf"), sep = ""))
...
# Extract canonical transcript annotations for target.gene from gene.gtf
system(paste("grep \"Ensembl_canonical\"", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/gene.gtf"), ">",
    file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/canonical.gtf")))

# Run stringtie on haplotype-specific BAM file of interest for CDG-P09
# For some unexplained reason, StringTie fails to discover the exon 4 skipping isoform if provided all gene annotations
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_2"), showWarnings = FALSE)
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P09"], "PIGQ/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/canonical.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))

# Merge output.gtf with gene.gtf via gffcompare
system(paste(gffcompare, "-M -D -T -C", "-o", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_1/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.updated.gtf")))

# Re-run stringtie on TEQUILA-seq BAM files
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P09"], "PIGQ/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_1/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P09"], "PIGQ/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/Haplotype_2/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
...
cohort.samples <- read.table(file.path(workdir, "CDG/samples.txt"), sep = "\t", header = TRUE) %>% filter(Provider != "Lan Lin" & ID != sample.map["PMD-C01"] & 
    ID != sample.map["CDG-P09"]) %>% pull(ID)
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "CDG", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
}
...
for (sample.id in c("Haplotype_1", "Haplotype_2", cohort.samples)) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/sqanti3_classification.txt"), 
...
# with the highest isoform fraction in haplotype 1 or average isoform fraction >= 0.1 across other cohort individuals
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
p1 <- ggplot() + geom_rect(data = utrDF, fill = "white", xmin = utrDF$V4, xmax = utrDF$V5, ymin = utrDF$Transcript_Number - 0.25,
    ...
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") + scale_fill_manual(values = c("#CBB3D0", "#FEAD63",
    "#F36C45", "#66C2A5", "#3188BD"))

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/Figure_4b.pdf"), plot = p1, width = 3, height = 1.5)
...
p2 <- ggplot() + geom_bar(data = summaryDF, aes(x = Group, y = Prop_Mean * 100, fill = Transcript_ID, group = Transcript_ID), linewidth = 0.5,
    ...
    jitter.height = 0, dodge.width = 0.75)) + theme_classic() + ylab("PIGQ isoform proportion (%)") + theme(panel.grid.major = element_blank(), 
    ...
    scale_fill_manual(values = c("#3188BD", "#66C2A5", "#F36C45", "#FEAD63", "#CBB3D0", "#BEBEBE")) + scale_color_manual(values = rep("black", 6)) + ylim(0, 100)

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/Figure_4c.pdf"), plot = p2, width = 3, height = 2.25)
...
levels(hapDF$Group) <- c("Haplotype 2\n(maternal)", "Haplotype 1\n(paternal)")
...
p3 <- ggplot() + geom_bar(data = hapDF, aes(x = Count, y = Group, fill = Transcript_ID), stat = "identity", linewidth = 0.5, position = "stack", width = 0.75) +
    ...
    legend.key.size = unit(0.3, "cm"), legend.title = element_blank(), legend.text = element_text(color = "black", size = 6)) + 
    scale_fill_manual(values = c("#3188BD", "#66C2A5", "#F36C45", "#FEAD63", "#CBB3D0", "#BEBEBE")) + xlim(0, 1000)

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_4/Figure_4d.pdf"), plot = p3, width = 3, height = 1.75)
