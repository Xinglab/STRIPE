...
system(paste(stringtie, file.path(workdir, "PMD/BS2-1/RNA/stripe/target_genes/EARS2/hap1_reads.bam"), "-o", 
    file.path(dirname(outfile), "tmp/BS2-1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
for (sample.id in cohort.samples) {
    ...
    system(paste(stringtie, file.path(dirname(outfile), "tmp", sample.id, "input.bam"), "-o", 
...
system(paste(stringtie, "--merge -G", file.path(dirname(outfile), "tmp/gene.gtf"), "-o", file.path(dirname(outfile), "tmp/merged.gtf"),
    "-i", file.path(dirname(outfile), "tmp", "BS2-1", "output.gtf")))
...
newTxAssign <- setNames(seq(length(keepTranscripts),1), c(9,4,7,3,2,1,8,6,5))
...
palette <- setNames(c("#5A8CA8", "#94C1B6", "#689E45", "#D6604D", "#D7869D", "#DFC27C", "#CCB1D4", "#686868", "#406F66"),
...
palette <- setNames(c("#5A8CA8", "#94C1B6", "#689E45", "#D6604D", "#D7869D", "#DFC27C", "#CCB1D4", "#686868", "#406F66", "#E2E2E2"),
