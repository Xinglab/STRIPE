...
# cryptic donor splice site within exon 3 for individual Q1663, other cohort individuals, and tissue-matched GTEx controls. 
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp')
    ...
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/input.gtf', sep = '\t', header = None)
    ...
    outDF.to_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/cohort.tsv', sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/Figure_3c.pdf")
...
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/cohort.tsv"), sep = "\t", 
    header = TRUE) %>% gather("Event", "PSI", -Sample_ID) %>% mutate(Group = ifelse(Sample_ID == "Q1663", "Patient", "Cohort individuals"),
