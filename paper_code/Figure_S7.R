...
# that removes the promoter region and first exon. (b) Minor haplotype expression ratios for TRMU across cohort samples 
...
p2 <- ggplot() + geom_jitter(data = outDF %>% filter(Label == ""), aes(x = Group, y = Ratio * 100, color = Group), stroke = NA, alpha = 0.5, height = 0, 
    ...
    axis.title.x = element_blank()) + ylab("Minor haplotype\nexpression ratio (%)") + scale_color_manual(values = c("#1A78AC", "#6FC7CE")) + ggtitle("TRMU")
