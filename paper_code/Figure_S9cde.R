...
# Date: 2025.10.08
# Supplementary Figure 9c,d,e

# (c) Full-length structures and (d) isoform-level proportions of NUBPL transcripts detected from TEQUILA-seq data of 
# individual PMD-P07 (resolved by haplotype) and other cohort individuals. Shaded and unshaded regions within transcript 
# structures represent putative coding sequences and untranslated regions respectively. Transcripts featuring NUBPL exon 
# 10 skipping with the highest isoform-level proportion in Haplotype 1, or transcripts featuring usage of a polyadenylation 
# site within intron 8 with the highest isoform-level proportion in Haplotype 2, or transcripts with an average isoform-level 
# proportion ≥ 10% across other cohort individuals are displayed in (c, d). (e) Read counts for NUBPL transcripts 
# displayed in (c, d) in individual PMD-P07 (resolved by haplotype). 
...
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf"), sep = ""))
...
# Run stringtie on haplotype-specific BAM files of interest for PMD-P07
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["PMD-P07"], "NUBPL/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_2"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["PMD-P07"], "NUBPL/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_2/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
system(paste(gffcompare, "-M -D -T -C", "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_1/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_2/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.updated.gtf")))
...
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["PMD-P07"], "NUBPL/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_1/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B"))
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["PMD-P07"], "NUBPL/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/Haplotype_2/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B"))

# Run stringtie on TEQUILA-seq BAM files for other cohort individuals
cohort.samples <- read.table(file.path(workdir, "PMD/samples.txt"), sep = "\t", header = TRUE) %>% filter((Provider == "Rebecca Ganetzky" & ID != sample.map["PMD-P07"]) | 
    (Provider == "Marni Falk" & Status != "Diagnosed" & !grepl("-MF", ID))) %>% pull(ID)
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "PMD", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B"))
}
...
for (sample.id in c("Haplotype_1", "Haplotype_2", cohort.samples)) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/sqanti3_classification.txt"), 
...
# Only keep transcripts featuring NUBPL exon 10 skipping with the highest isoform-level proportion in Haplotype 1, or transcripts featuring usage of a 
# polyadenylation site within intron 8 with the highest isoform-level proportion in Haplotype 2, or transcripts with an average isoform-level proportion ≥ 10% 
# across other cohort individuals
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/Figure_S9c.pdf"), plot = p1, width = 3, height = 1.25)
...
ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/Figure_S9d.pdf"), plot = p2, width = 3, height = 2.25)
...
p3 <- ggplot() + geom_bar(data = hapDF, aes(x = Count, y = Group, fill = Transcript_ID), stat = "identity", linewidth = 0.5, position = "stack", width = 0.75) +
    ...
    legend.key.size = unit(0.3, "cm"), legend.title = element_blank(), legend.text = element_text(color = "black", size = 6)) + 
    scale_fill_manual(values = c("#3188BD", "#FCAE62", "#CBB3D0", "#BEBEBE")) + xlim(0, 600)

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/Figure_S9e.pdf"), plot = p3, width = 3, height = 1.75)

# Remove intermediate files
system(paste("rm -rf", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S9/tmp")))
