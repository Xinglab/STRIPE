...
# PMD-P11 (resolved by haplotype), other cohort individuals, and 504 tissue-matched GTEx controls. 
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp')
    sampleDF = pd.read_csv(workdir + '/PMD/samples.txt', sep = '\t', header = 0)
    sampleDF = sampleDF[((sampleDF['Provider'] == 'Rebecca Ganetzky') & (sampleDF['ID'] != sampleMap['PMD-P11'])) | ((sampleDF['Provider'] == 'Marni Falk') & 
    ...
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/input.gtf', sep = '\t', header = None)
    ...
    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/PMD/' + sampleMap['PMD-P11'] + '/RNA/stripe/target_genes/SUCLG1/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'SUCLG1'].values[0][0],
            targetDF[targetDF[4] == 'SUCLG1'].values[0][1], targetDF[targetDF[4] == 'SUCLG1'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
    ...
    pd.DataFrame(output.items(), columns = ['Sample_ID', 'PSI']).to_csv(workdir + '/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/cohort.tsv', 
        sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/Figure_S10b.pdf")
...
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/cohort.tsv"), sep = "\t", 
