...
# (b) Usage frequencies for a splice junction involving OPA1 exon 14 skipping in individual Q2032s1 (resolved by haplotype), 
...
# Used the following Python code to retrieve read support and total coverage for splice junction chr3:193643050-193643527
# in cohort samples
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/tmp')
    sampleDF = pd.read_csv(workdir + '/PMD/samples.txt', sep = '\t', header = 0)
    sampleDF = sampleDF[((sampleDF['Provider'] == 'Rebecca Ganetzky') & (sampleDF['ID'] != 'Q2032s1')) | ((sampleDF['Provider'] == 'Marni Falk') & 
    ...
    gene_id = targetDF[targetDF[4] == 'OPA1'].values[0][3]
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/tmp/input.gtf', sep = '\t', header = None)
    ...
    for sampid in sampleDF['ID']:
        ...
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'OPA1'].values[0][0],
            targetDF[targetDF[4] == 'OPA1'].values[0][1], targetDF[targetDF[4] == 'OPA1'].values[0][2]):
        ...
        junctions = junctions[(junctions['Start'] == '193643050') | (junctions['End'] == '193643527')].merge(
            pd.DataFrame([['chr3', '193643050', '193643527']], columns = ['Chrom', 'Start', 'End']), how = 'outer').fillna(0)
        junctions['PSI'] = junctions['Read_Counts']/(np.nan if junctions['Read_Counts'].sum() < 20 else junctions['Read_Counts'].sum())
        output[sampid] = junctions[(junctions['Start'] == '193643050') & (junctions['End'] == '193643527')]['PSI'].item()

    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/PMD/Q2032s1/RNA/stripe/target_genes/OPA1/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'OPA1'].values[0][0],
            targetDF[targetDF[4] == 'OPA1'].values[0][1], targetDF[targetDF[4] == 'OPA1'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
        ...
        junctions = junctions[(junctions['Start'] == '193643050') | (junctions['End'] == '193643527')].merge(
            pd.DataFrame([['chr3', '193643050', '193643527']], columns = ['Chrom', 'Start', 'End']), how = 'outer').fillna(0)
        junctions['PSI'] = junctions['Read_Counts']/(np.nan if junctions['Read_Counts'].sum() < 20 else junctions['Read_Counts'].sum())
        output['Haplotype_' + str(idx)] = junctions[(junctions['Start'] == '193643050') & (junctions['End'] == '193643527')]['PSI'].item()

    pd.DataFrame(output.items(), columns = ['Sample_ID', 'PSI']).to_csv(workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/tmp/cohort.tsv', 
        sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/Figure_S10b.pdf")

# Retrieve read support and total coverage for splice junction chr3:193643050-193643527 in GTEx controls
gtex.splice <- full_join(read.table(file.path(workdir, "PMD/references/GTEx_v8/Fibroblast/target_genes.junction_counts.txt"), 
    ...
    filter(Chrom == "chr3" & (Start == "193643050" | End == "193643527")) %>% distinct(), tibble(Chrom = "chr3", 
    Start = "193643050", End = "193643527"), by = join_by(Chrom, Start, End)) %>% replace(is.na(.), 0)
junction.counts <- as.numeric(gtex.splice %>% filter(Chrom == "chr3" & Start == "193643050" & End == "193643527") %>% 
...
# Retrieve read support and total coverage for splice junction chr3:193643050-193643527 in cohort samples
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S10/tmp/cohort.tsv"), sep = "\t", 
...
p <- ggplot() + geom_bar(data = summaryDF, aes(x = PSI * 100, y = Group, fill = Group), stat = "identity", width = 0.75) + geom_point(data = outDF,
    ...
    height = 0.2)) + theme_classic() + xlab("OPA1 exon 14 skipping (%)") + theme(panel.grid.major = element_blank(), 
