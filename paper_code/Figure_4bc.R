...
outfile1 <- file.path(workdir, "manuscript/Main_Figures/Figure_4/Figure_4b.pdf")
outfile2 <- file.path(workdir, "manuscript/Main_Figures/Figure_4/Figure_4c.pdf")

# =====================================================================================================================
#                                                      PANEL B
# =====================================================================================================================

# Create a temporary directory in the folder for Figure 4
...
# Run stringtie on haplotype-specific BAM file for CDG-132-1
system(paste(stringtie, file.path(workdir, "CDG/CDG-132-1/RNA/stripe/target_genes/PIGQ/hap1_reads.bam"), "-G", 
    file.path(dirname(outfile1), "tmp/gene.gtf"), "-o", file.path(dirname(outfile1), "tmp/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
system(paste(stringtie, "--merge -G", file.path(dirname(outfile1), "tmp/gene.gtf"), "-o", file.path(dirname(outfile1), "tmp/merged.gtf"), 
    "-i", file.path(dirname(outfile1), "tmp/output.gtf")))
...
# Estimate abundances of transcripts in merged.updated.gtf from haplotype-specific BAM file for CDG-132-1
dir.create(file.path(dirname(outfile1), "tmp/CDG-132-1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "CDG/CDG-132-1/RNA/stripe/target_genes/PIGQ/hap1_reads.bam"), "-G", 
    file.path(dirname(outfile1), "tmp/merged.updated.gtf"), "-o", file.path(dirname(outfile1), "tmp/CDG-132-1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
for (sample.id in cohort.samples) {
    ...
    system(paste(stringtie, file.path(dirname(outfile1), "tmp", sample.id, "input.bam"), "-G", file.path(dirname(outfile1), "tmp/merged.updated.gtf"), 
        "-o", file.path(dirname(outfile1), "tmp", sample.id, "output.gtf"), "-L -u -M 0"))
}

# Assemble an TPM matrix for transcripts in merged.updated.gtf
outDF <- read.table(file.path(dirname(outfile1), "tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
outDF <- left_join(outDF, read.table(file.path(dirname(outfile1), "tmp/CDG-132-1/output.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
    ...
    setNames(c("Transcript_ID", "CDG-132-1")), by = join_by(Transcript_ID)) %>% replace(is.na(.), 0)

for (sample.id in cohort.samples) {
    outDF <- left_join(outDF, read.table(file.path(dirname(outfile1), "tmp", sample.id, "output.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
        filter(V3 == "transcript") %>% mutate(Transcript_ID = unlist(lapply(V9, function(x) PullFeature(x, "reference_id"))),
        TPM = as.numeric(unlist(lapply(V9, function(x) gsub(";", "", PullFeature(x, "TPM")))))) %>% select(Transcript_ID, TPM) %>% drop_na %>%
...
gtfDF <- read.table(file.path(dirname(outfile1), "tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
palette <- c("#598CA8", "#94C1B6", "#D6604D", "#DFC17D", "#D7869D")
...
ggsave(outfile1, plot = p1, width = 3.5, height = 1.75)
...
sampleOrder <- propDF %>% filter(Transcript_ID == "ENST00000470411.2") %>% arrange(Proportion) %>% pull(Sample_ID)
propDF$Sample_ID <- factor(propDF$Sample_ID, levels = sampleOrder)

p2 <- ggplot(propDF %>% mutate(Transcript_ID = gsub("MSTRG", "NovelTx", Transcript_ID)), aes(x = Proportion, y = Sample_ID, fill = Transcript_ID)) + 
    ...
    plot.margin = margin(r = 20, unit = "pt"), legend.position = "bottom") + scale_fill_manual(values = c(palette, "#BFBFBF")) +
    guides(fill = guide_legend(ncol = 2))

ggsave(outfile2, plot = p2, width = 2.5, height = 3.75)
