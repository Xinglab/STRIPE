...
system(paste(stringtie, file.path(workdir, "CDG/CDG-132-1/RNA/stripe/target_genes/PIGQ/hap1_reads.bam"), "-o", file.path(dirname(outfile1), 
    "tmp/CDG-132-1/output.gtf"), "-L -s 5 -c 5 -u -M 0 -l CDG-132-1"))
...
keepTranscripts <- unique(c(propMatrix$Transcript_ID[propMatrix[,2] >= 0.02],
...
newTxAssign <- setNames(seq(length(keepTranscripts),1), c(8,7,3,4,1,5,6,13,12,14,9,16,11,10,15,2))
...
palette <- setNames(c(brewer.pal(9, "Blues")[7:3], brewer.pal(9, "GnBu")[5:4], brewer.pal(9, "YlOrBr")[5:2], brewer.pal(9, "Reds")[3:6], brewer.pal(9, "Purples")[6], "#BDBDBD"), seq(length(keepTranscripts), 1))
...
sortTx <- c(as.character(setNames(unique(gtfDF$Transcript_ID[!grepl("ENST", gtfDF$Transcript_ID)]), paste("NovelTx", 1:length(unique(gtfDF$Transcript_ID[!grepl("ENST", 
    gtfDF$Transcript_ID)])), sep = "."))[c("NovelTx.9", "NovelTx.10", "NovelTx.8", "NovelTx.6", "NovelTx.5", "NovelTx.11", "NovelTx.7")]), "ENST00000470411.2")
sampleOrder <- propDF %>% filter(Transcript_ID %in% sortTx) %>% group_by(Sample_ID) %>% summarise(Proportion = sum(Proportion)) %>%
    ungroup %>% arrange(Proportion) %>% pull(Sample_ID)
propDF$Sample_ID <- factor(propDF$Sample_ID, levels = sampleOrder)

palette <- setNames(c(brewer.pal(9, "Blues")[7:3], brewer.pal(9, "GnBu")[5:4], brewer.pal(9, "YlOrBr")[5:2], brewer.pal(9, "Reds")[3:6], brewer.pal(9, "Purples")[6], "#BDBDBD"), 
