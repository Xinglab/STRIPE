...
outfile <- file.path(workdir, "manuscript/Supplementary_Figures/Figure_S11/Figure_S11bc.pdf")
...
dir.create(file.path(dirname(outfile), "tmp/CDG-147-1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "CDG/CDG-147-1/RNA/stripe/target_genes/PIGN/hap1_reads.bam"), "-G", file.path(dirname(outfile), "tmp/gene.gtf"), 
    "-o", file.path(dirname(outfile), "tmp/CDG-147-1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
# Generate a merged GTF using transcripts from GENCODE and transcripts discovered for CDG-147-1
# Stringtie merge fails here since isoforms using the cryptic polyadenylation site within intron 11 are incorrectly merged with other transcripts
system(paste("cat", file.path(dirname(outfile), "tmp/gene.gtf"), file.path(dirname(outfile), "tmp/CDG-147-1/output.gtf"), ">",
    file.path(dirname(outfile), "tmp/merged.gtf")))
...
system(paste(stringtie, file.path(workdir, "CDG/CDG-147-1/RNA/stripe/target_genes/PIGN/hap1_reads.bam"), "-G", 
    file.path(dirname(outfile), "tmp/merged.gtf"), "-o", file.path(dirname(outfile), "tmp/CDG-147-1/output.gtf"), 
    "-s 5 -c 5 -u -M 0 -e -B"))
for (sample.id in cohort.samples) {
...
system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(dirname(outfile), "tmp/CDG-147-1/output.gtf"), "-a", 
...
sampleTx <- propMatrix[,1:2] %>% arrange(desc(`CDG-147-1`))
cohortTx <- propMatrix[,-2] %>% mutate(Total = rowSums(across(where(is.numeric)))) %>% select(Transcript_ID, Total) %>% arrange(desc(Total))
keepTranscripts <- c("ENST00000640876.1", "ENST00000400334.7", "ENST00000357637.10", "ENST00000640252.2", "STRG.1.3", "STRG.1.5", "STRG.1.4", "STRG.1.2")
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:3], brewer.pal(9, "YlOrBr")[5:3], brewer.pal(9, "Reds")[4]), seq(length(keepTranscripts), 1))
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:3], brewer.pal(9, "YlOrBr")[5:3], brewer.pal(9, "Reds")[4], "#BDBDBD"), c(recode(keepTranscripts, !!!setNames(paste("NovelTx", 
...
ggsave(outfile, plot = p, width = 4.75, height = 2)
