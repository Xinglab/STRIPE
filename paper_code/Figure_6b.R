...
# Usage frequencies for splice junctions involving MTFMT exon 4 skipping and inclusion of a 53 bp pseudoexon between 
# MTFMT exons 6 and 7 in individual Q1507 (resolved by haplotype), other cohort individuals, and tissue-matched GTEx controls.
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp')
    ...
    sampleDF = sampleDF[(sampleDF['Provider'] == 'Rebecca Ganetzky') | ((sampleDF['Provider'] == 'Marni Falk') & 
        (sampleDF['Status'] != 'Diagnosed') & ~sampleDF['ID'].str.contains('-MF') & (sampleDF['ID'] != 'Q1507'))]
    targetDF = pd.read_csv(workdir + '/PMD/references/target_genes.bed', sep = '\t', header = None)
    gene_id = targetDF[targetDF[4] == 'MTFMT'].values[0][3]
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/input.gtf', sep = '\t', header = None)
    ...
    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'MTFMT'].values[0][0],
            targetDF[targetDF[4] == 'MTFMT'].values[0][1], targetDF[targetDF[4] == 'MTFMT'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
    ...
    outDF.to_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/cohort.tsv', sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_6/Figure_6b.pdf")
...
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_6/tmp/cohort.tsv"), sep = "\t", 
