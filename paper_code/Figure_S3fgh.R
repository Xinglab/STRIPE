...
# Date: 2025.10.07
# Supplementary Figure 3f,g,h

# (f, g) Principal component analysis (PCA) based on usage frequencies for annotated splice junctions in target genes from the 
# CDG-466 gene panel (left) and the PMD-359 gene panel (right) in fibroblasts from 88 cohort individuals and GTEx samples 
# for clinically accessible tissues focusing on (f) PC3 and PC4 and (g) PC5 and PC6. (h) Percentage of variance explained by 
# the first ten principal components for splice junction usage frequencies in target genes from the CDG-466 gene panel (left) 
# and the PMD-359 gene panel (right). 
...
ggsave(file.path(outdir, "Figure_S3f.pdf"), plot = p, width = 4.5, height = 1.75)
...
ggsave(file.path(outdir, "Figure_S3g.pdf"), plot = p, width = 4.5, height = 1.75)
...
ggsave(file.path(outdir, "Figure_S3h.pdf"), plot = p, width = 3, height = 1.75)