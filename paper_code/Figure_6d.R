...
# Number of uniquely mapped and multi-mapped TEQUILA-seq reads from patient CDG-0367 that are aligned to ALG1 and its 
# known pseudogenes. 
...
outfile <- file.path(workdir, "manuscript/Main_Figures/Figure_6/Figure_6d.pdf")

# Pull out all reads supporting ALG1 and its pseudogenes
dir.create(file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp"), showWarnings = FALSE)
...
for (i in 1:length(gene.list)) {
    ...
    system(paste("grep", names(gene.list[i]), gencode, ">", file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/input.gtf")))
    gene.coord <- read.table(file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/input.gtf"), sep = "\t",
        header = FALSE, quote = "") %>% filter(V3 == "gene") %>% reframe(Coord = paste(V1, ":", V4, 
        "-", V5, sep = "")) %>% pull(Coord)
    
    # Pull out TEQUILA-seq reads mapping to gene region
    system(paste(samtools, "view -hb", file.path(workdir, "CDG/CDG-0367/RNA/CDG-0367_TEQUILA.bam"), gene.coord,
        ">", file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/input.bam")))
    system(paste(samtools, "flagstat -O tsv", file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/input.bam"), ">",
        file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/flagstat.txt")))
    ...
    flagDF <- read.table(file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/flagstat.txt"), sep = "\t", header = FALSE)
...
p <- ggplot(outDF, aes(x = Count, y = Gene, fill = Category, group = Category)) + geom_bar(position = "stack", width = 0.75, stat = "identity") + 
    ...
    legend.key.size = unit(0.3, "cm")) + xlab("Number of TEQUILA-seq reads (CDG-0367)") + scale_x_continuous(limits = c(0, 20000)) +
...
system(paste("rm", file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp/*")))
system(paste("rmdir", file.path(workdir, "manuscript/Main_Figures/Figure_6/tmp")))
