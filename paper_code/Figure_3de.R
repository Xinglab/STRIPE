...
# individual Q1663 and other cohort individuals. Shaded and unshaded regions within transcript structures represent 
...
dir.create(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/gene.gtf"), sep = ""))

# Run stringtie on TEQUILA-seq BAM file for Q1663
...
dir.create(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/Q1663"), showWarnings = FALSE)
system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "PMD", "Q1663", "RNA", paste("Q1663", "TEQUILA.bam", sep = "_")), 
    paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", 
    "Q1663", "input.bam")))
system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", "Q1663", "input.bam")))
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", "Q1663", "input.bam"), "-G",
    file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/gene.gtf"), "-E 10", "-o", file.path(workdir, 
    "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", "Q1663", "output.gtf"), "-L -s 5 -c 5 -u -M 0"))

# Merge output.gtf with gene.gtf via gffcompare
system(paste(gffcompare, "-M -D -T -C -X", "-o", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/Q1663/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged.updated.gtf")))
...
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "PMD", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
}

# Construct FPKM matrix across samples
outDF <- tibble(Transcript_ID = character())
for (sample.id in cohort.samples) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/sqanti3_classification.txt"), 
...
sampleTx <- propMatrix %>% select(Transcript_ID, Q1663) %>% arrange(desc(Q1663))
cohortTx <- propMatrix %>% select(-Q1663) %>% mutate(Total = rowSums(across(where(is.numeric)))) %>% select(Transcript_ID, Total) %>% arrange(desc(Total))
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
ggsave(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/Figure_3d.pdf"), plot = p1, width = 3, height = 1.5)
...
propDF <- propDF %>% mutate(Transcript_ID = factor(recode(Transcript_ID, !!!setNames(paste("NovelTx", 1:length(unique(gtfDF$Transcript_ID[!grepl("ENST", 
    ...
    pull(Transcript_ID), "Other")), Group = factor(ifelse(Sample_ID == "Q1663", "Patient", "Cohort individuals"), 
...
system(paste("rm -rf", file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/tmp")))

ggsave(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_3/Figure_3e.pdf"), plot = p2, width = 3, height = 2.5)