...
# (b) Usage frequencies for a splice junction involving PMM2 exon 5 skipping in individual 11547 (resolved by haplotype), 
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/tmp')
    sampleDF = pd.read_csv(workdir + '/CDG/samples.txt', sep = '\t', header = 0)
    sampleDF = sampleDF[(sampleDF['Provider'] != 'Lan Lin') & ~sampleDF['ID'].isin({'E1877', '11547'})]
    targetDF = pd.read_csv(workdir + '/CDG/references/target_genes.bed', sep = '\t', header = None)
    gene_id = targetDF[targetDF[4] == 'PMM2'].values[0][3]
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/tmp/input.gtf', sep = '\t', header = None)
    ...
    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/11547/PMM2/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'PMM2'].values[0][0],
            targetDF[targetDF[4] == 'PMM2'].values[0][1], targetDF[targetDF[4] == 'PMM2'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
    ...
    pd.DataFrame(output.items(), columns = ['Sample_ID', 'PSI']).to_csv(workdir + '/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/tmp/cohort.tsv', 
        sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/Figure_S9b.pdf")
...
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S9/tmp/cohort.tsv"), sep = "\t", 
