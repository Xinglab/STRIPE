...
# (d) Principal component analysis (PCA) based on usage frequencies for annotated splice junctions in target genes from the 
# CDG-466 gene panel (left) and the PMD-359 gene panel (right) in fibroblasts from 88 cohort individuals and GTEx fibroblasts. 
# (e) Comparison of median usage frequencies for annotated splice junctions in target genes from the CDG-466 gene panel (left) 
# and PMD-359 gene panel (right) between cohort and GTEx fibroblasts
...
medianDF <- tibble(Panel = character(), Junction = character(), GTEX = numeric(), Cohort = numeric())
...
for (disease in c("CDG", "PMD")) {
    ...
    # Calculate usage frequencies for annotated junctions from target genes for GTEx fibroblasts
    tissueDF <- read.table(file.path(workdir, disease, "references/GTEx_v8/Fibroblast/target_genes.junction_counts.txt"), sep = "\t",
    ...
    tissue.assign <- c(tissue.assign, setNames(rep("Fibroblasts (GTEx v8)", ncol(tissueDF) - 3), colnames(tissueDF)[-(1:3)]))
    ...
    # Compute median junction usage among GTEx and cohort fibroblasts
    medianDF <- bind_rows(medianDF, full_join(tibble::enframe(apply(outDF %>% select(all_of(colnames(outDF)[grepl("GTEX", colnames(outDF))])), 1, median)) %>%
        setNames(c("Junction", "GTEX")) %>% mutate(Panel = disease, .before = 1), tibble::enframe(apply(outDF %>% 
        select(all_of(colnames(outDF)[!grepl("GTEX", colnames(outDF))])), 1, median)) %>% setNames(c("Junction", "Cohort")) %>% 
        mutate(Panel = disease, .before = 1), by = join_by(Panel, Junction)) %>% drop_na())
}
...
medianDF$Panel <- factor(medianDF$Panel, levels = c("CDG", "PMD"))
levels(medianDF$Panel) <- c("CDG-466 gene panel", "PMD-359 gene panel")
...
p <- ggplot(medianDF, aes(x = GTEX*100, y = Cohort*100)) + facet_wrap(. ~ Panel, nrow = 1) + geom_bin_2d(bins = 20) + theme_classic() +
    scale_fill_gradientn(colors = rev(colorspace::sequential_hcl(5, "GnBu")), name = "Number of junctions", trans = "log10") +
    xlab("Fibroblasts (GTEx v8)") + ylab("Fibroblasts (cohort)") + theme(panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(), panel.background = element_blank(), axis.text = element_text(color = "black", size = 6), 
    axis.title = element_text(color = "black", size = 7), axis.ticks = element_line(color = "black", linewidth = 0.25), 
    axis.line = element_line(color = "black", linewidth = 0.25), strip.text = element_text(color = "black", size = 6),
    legend.text = element_text(color = "black", size = 6), legend.title = element_text(color = "black", size = 7), 
    legend.key.size = unit(0.3, 'cm')) + 

ggsave(file.path(outdir, "Figure_S3e.pdf"), plot = p, width = 4.25, height = 1.75)
