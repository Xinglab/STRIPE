...
system(paste(stringtie, "/mnt/isilon/lin_lab_share/TEQUILA-Dx/CDG/rna_seq/AnJa/examples/PIGN/hap2_reads.bam", "-G", file.path(dirname(outfile), "tmp/gene.gtf"), 
    "-o", file.path(dirname(outfile), "tmp/AnJa/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
system(paste(stringtie, file.path(workdir, "CDG/CDG-161-1/RNA/stripe/target_genes/PIGN/hap2_reads.bam"), "-G", file.path(dirname(outfile), "tmp/gene.gtf"), 
    "-o", file.path(dirname(outfile), "tmp/CDG-161-1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))
...
data.table::fwrite(list(file.path(dirname(outfile), "tmp", c("AnJa", "CDG-161-1", cohort.samples), "output.gtf")), 
...
system(paste(stringtie, "/mnt/isilon/lin_lab_share/TEQUILA-Dx/CDG/rna_seq/AnJa/examples/PIGN/hap2_reads.bam", "-G", 
    file.path(dirname(outfile), "tmp/merged.gtf"), "-o", file.path(dirname(outfile), "tmp/AnJa/output.gtf"), 
    "-s 5 -c 5 -u -M 0 -e -B"))
system(paste(stringtie, file.path(workdir, "CDG/CDG-161-1/RNA/stripe/target_genes/PIGN/hap2_reads.bam"), "-G", 
    file.path(dirname(outfile), "tmp/merged.gtf"), "-o", file.path(dirname(outfile), "tmp/CDG-161-1/output.gtf"), 
    "-s 5 -c 5 -u -M 0 -e -B"))
for (sample.id in cohort.samples) {
...
keepTranscripts <- c("ENST00000400334.7", "ENST00000357637.10", "ENST00000640252.2", "ENST00000638329.1", "MSTRG.1.7", "MSTRG.1.8")
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:4], brewer.pal(9, "YlOrBr")[4], brewer.pal(9, "Reds")[3:4]), seq(length(keepTranscripts), 1))
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:4], brewer.pal(9, "YlOrBr")[4], brewer.pal(9, "Reds")[3:4], "#BDBDBD"), c(recode(keepTranscripts, !!!setNames(paste("NovelTx", 
