...
# (c) Full-length structures and (d) isoform-level proportions of PIGN transcripts detected from TEQUILA-seq data of 
...
outfile <- file.path(workdir, "manuscript/Supplementary_Figures/Figure_S10/Figure_S10cd.pdf")

# =====================================================================================================================
#                                                      PANEL C
# =====================================================================================================================

# Create a temporary directory in the folder for Supplementary Figure 10
...
system(paste(stringtie, "/mnt/isilon/lin_lab_share/TEQUILA-Dx/CDG/rna_seq/AnJa/examples/PIGN/hap2_reads.bam", "-G", file.path(dirname(outfile), "tmp/canonical.gtf"), 
    "-o", file.path(dirname(outfile), "tmp/AnJa/output.gtf"), "-L -s 5 -c 5 -u -M 0 -l AnJa"))
...
system(paste(stringtie, file.path(workdir, "CDG/CDG-161-1/RNA/stripe/target_genes/PIGN/hap2_reads.bam"), "-G", file.path(dirname(outfile), "tmp/canonical.gtf"), 
    "-o", file.path(dirname(outfile), "tmp/CDG-161-1/output.gtf"), "-L -s 5 -c 5 -u -M 0 -l CDG-161-1"))
...
for (sample.id in cohort.samples) {
    ...
    system(paste(stringtie, file.path(dirname(outfile), "tmp", sample.id, "input.bam"), "-G", file.path(dirname(outfile), "tmp/canonical.gtf"), "-o", 
...
for (sample.id in c("AnJa", "CDG-161-1", cohort.samples)) {
...
sampleTx <- propMatrix[,1:3] %>% mutate(Total = rowSums(across(where(is.numeric)))) %>% select(Transcript_ID, Total) %>% arrange(desc(Total))
cohortTx <- propMatrix[,-c(2,3)] %>% mutate(Total = rowSums(across(where(is.numeric)))) %>% select(Transcript_ID, Total) %>% arrange(desc(Total))
keepTranscripts <- c("CFB-CHOP-1.2.3", "ENST00000640876.1", "ENST00000640252.2", "ENST00000400334.7", "AnJa.1.12", "AnJa.1.1", "AnJa.1.2")
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:3], brewer.pal(9, "YlOrBr")[4], brewer.pal(9, "Reds")[3:4]), seq(length(keepTranscripts), 1))
...
palette <- setNames(c(brewer.pal(9, "Blues")[6:3], brewer.pal(9, "YlOrBr")[4], brewer.pal(9, "Reds")[3:4], "#BDBDBD"), c(recode(keepTranscripts, !!!setNames(paste("NovelTx", 
...
propDF <- propDF %>% mutate(Transcript_ID = factor(recode(Transcript_ID, !!!setNames(paste("NovelTx", 1:length(unique(gtfDF$Transcript_ID[!grepl("ENST", 
    ...
    Group = factor(ifelse(Sample_ID == "AnJa", "AnJa\n(haplotype 2)", ifelse(Sample_ID == "CDG-161-1", "CDG-161-1\n(haplotype 2)", "Fibroblasts\n(cohort)")), 
    levels = c("Fibroblasts\n(cohort)", "AnJa\n(haplotype 2)", "CDG-161-1\n(haplotype 2)")))
...
p2 <- ggplot() + geom_bar(data = summaryDF, aes(x = Group, y = Prop_Mean * 100, color = Transcript_ID, group = Transcript_ID), fill = NA, linewidth = 0.5,
    ...
    ggtitle("PIGN") + scale_color_manual(values = palette) + guides(color = guide_legend(ncol = 2)) + ylim(0, 100)
