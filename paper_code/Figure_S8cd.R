...
# individual BS2-1 (resolved by haplotype) and other cohort individuals. Shaded and unshaded regions within transcript 
...
dir.create(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), sep = ""))

# Run stringtie on haplotype-specific BAM file of interest for BS2-1
dir.create(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_2"), showWarnings = FALSE)
dir.create(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/BS2-1/EARS2/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))

# Merge output.gtf with gene.gtf via gffcompare
system(paste(gffcompare, "-M -D -T -C -X", "-o", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.updated.gtf")))

# Re-run stringtie on TEQUILA-seq BAM files
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/BS2-1/EARS2/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/BS2-1/EARS2/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/Haplotype_2/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))

# Run stringtie on TEQUILA-seq BAM files for other cohort individuals
cohort.samples <- read.table(file.path(workdir, "PMD/samples.txt"), sep = "\t", header = TRUE) %>% filter((Provider == "Rebecca Ganetzky" & ID != "BS2-1") | 
    (Provider == "Marni Falk" & Status != "Diagnosed" & !grepl("-MF", ID))) %>% pull(ID)
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "PMD", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
}
...
for (sample.id in c("Haplotype_1", "Haplotype_2", cohort.samples)) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/sqanti3_classification.txt"), 
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
ggsave(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/Figure_S8c.pdf"), plot = p1, width = 3, height = 0.75)
...
p2 <- ggplot() + geom_bar(data = summaryDF, aes(x = Group, y = Prop_Mean * 100, fill = Transcript_ID, group = Transcript_ID), linewidth = 0.5,
    ...
    jitter.height = 0, dodge.width = 0.75)) + theme_classic() + ylab("EARS2 isoform proportion (%)") + theme(panel.grid.major = element_blank(), 
...
system(paste("rm -rf", file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/tmp")))

ggsave(file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S8/Figure_S8d.pdf"), plot = p2, width = 3, height = 2)