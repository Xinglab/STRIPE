...
# Haplotype-resolved read coverage over PIGN in patient CDG-183-1, who is heterozygous for a stop-gain variant 
...
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_2/Figure_2b.pdf")
...
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/CDG-183-1/PIGN/hap1_reads.bam"),
    ">", file.path(dirname(outfile), "tmp/hap1_coverage.txt")))
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/CDG-183-1/PIGN/hap2_reads.bam"),
...
gene.coverage <- ggplot(plotDF, aes(x = 1 - Coord, y = Coverage, color = Group)) + 
    ...
    ylab("Read coverage") + scale_color_manual(values = c("#EC7B02", "#90AFB1")) + ylim(0, 300) + 
