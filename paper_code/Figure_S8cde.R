...
# Date: 2025.10.08
# Supplementary Figure 8c,d,e

# (c) Full-length structures and (d) isoform-level proportions of PMM2 transcripts detected from TEQUILA-seq data of 
# individual CDG-P10 (resolved by haplotype) and other cohort individuals. Shaded and unshaded regions within transcript 
# structures represent putative coding sequences and untranslated regions respectively. Transcripts featuring PMM2 exon 5 
# skipping with the highest isoform-level proportion in Haplotype 1 or transcripts with an average isoform-level proportion 
# ≥ 10% across other cohort individuals are displayed in (c, d). (e) Read counts for PMM2 transcripts displayed in (c, d) 
# in individual CDG-P10 (resolved by haplotype). 
...
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), sep = ""))
...
# Run stringtie on haplotype-specific BAM file of interest for CDG-P10
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_2"), showWarnings = FALSE)
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_1"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P10"], "PMM2/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), "-L -s 5 -c 5 -u -M 0"))

# Merge output.gtf with gene.gtf via gffcompare
system(paste(gffcompare, "-M -D -T -C -X", "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.updated.gtf")))

# Re-run stringtie on TEQUILA-seq BAM files
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P10"], "PMM2/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_1/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Tables/Table_S6", sample.map["CDG-P10"], "PMM2/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/Haplotype_2/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
...
cohort.samples <- read.table(file.path(workdir, "CDG/samples.txt"), sep = "\t", header = TRUE) %>% filter(Provider != "Lan Lin" & ID != sample.map["PMD-C01"] & 
    ID != sample.map["CDG-P10"]) %>% pull(ID)
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "CDG", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
}
...
for (sample.id in c("Haplotype_1", "Haplotype_2", cohort.samples)) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/sqanti3_classification.txt"), 
...
# Only keep transcripts featuring PMM2 exon 5 skipping with the highest isoform fraction in haplotype 1 or average isoform fraction >= 0.1 across other cohort individuals
keepTranscripts <- c("ENST00000268261.9", "ENST00000566604.5", "ENST00000566540.5")
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
labelDF <- gtfDF %>% select(Transcript_ID, Transcript_Number) %>% distinct
...
p1 <- ggplot() + geom_rect(data = utrDF, fill = "white", xmin = utrDF$V4, xmax = utrDF$V5, ymin = utrDF$Transcript_Number - 0.25,
    ...
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") + scale_fill_manual(values = c("#FCAE62", "#F36C45", "#3188BD"))

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/Figure_S8c.pdf"), plot = p1, width = 3, height = 1)
...
propDF <- propDF %>% mutate(Transcript_ID = factor(Transcript_ID, levels = c(labelDF %>% arrange(desc(Transcript_Number)) %>%
...
p2 <- ggplot() + geom_bar(data = summaryDF, aes(x = Group, y = Prop_Mean * 100, fill = Transcript_ID, group = Transcript_ID), linewidth = 0.5,
    ...
    scale_fill_manual(values = c("#3188BD", "#F36C45", "#FCAE62", "#BEBEBE")) + scale_color_manual(values = c("black", "black", "black", "black")) + 
    ylim(0, 100) + guides(fill = guide_legend(nrow = 2))

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/Figure_S8d.pdf"), plot = p2, width = 3, height = 2.25)
...
hapDF <- outDF %>% select("Transcript_ID", "Haplotype_1", "Haplotype_2") %>% gather("Sample_ID", "Count", -Transcript_ID) %>%
    ...
    mutate(Group = factor(gsub("_", " ", Sample_ID), levels = c("Haplotype 2", "Haplotype 1")))
...
p3 <- ggplot() + geom_bar(data = hapDF, aes(x = Count, y = Group, fill = Transcript_ID), stat = "identity", linewidth = 0.5, position = "stack", width = 0.75) +
    ...
    legend.key.size = unit(0.3, "cm"), legend.title = element_blank(), legend.text = element_text(color = "black", size = 6)) + 
    scale_fill_manual(values = c("#3188BD", "#F36C45", "#FCAE62", "#BEBEBE")) + xlim(0, 6000)

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/Figure_S8e.pdf"), plot = p3, width = 3, height = 1.5)

# Remove intermediate files
system(paste("rm -rf", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S8/tmp")))
