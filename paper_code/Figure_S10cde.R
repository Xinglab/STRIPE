...
# Date: 2025.10.08
# Supplementary Figure 10c,d,e

# (c) Full-length structures and (d) isoform-level proportions of SUCLG1 transcripts detected from TEQUILA-seq data of 
# individual PMD-P11 (resolved by haplotype) and other cohort individuals. Shaded and unshaded regions within transcript 
# structures represent putative coding sequences and untranslated regions respectively. Transcripts featuring usage of a 
# cryptic acceptor splice site in SUCLG1 intron 1 with the highest isoform-level proportion in Haplotype 2 or transcripts 
# with an average isoform-level proportion ≥ 10% across other cohort individuals are displayed in (c, d). (e) Read counts 
# for SUCLG1 transcripts displayed in (c, d) in individual PMD-P11 (resolved by haplotype). 
...
target.gene <- read.table(file.path(workdir, "PMD/references/target_genes.bed"), sep = "\t", header = FALSE) %>% filter(V5 == "SUCLG1")
gencode.gtf <- "/scr1/users/wangr5/references/gencode.v45.annotation.gtf"

# Create a temporary directory
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp"), showWarnings = FALSE)

# Extract transcript annotations for target.gene from gencode.gtf
system(paste("grep \"", target.gene$V4, "\" ", gencode.gtf, " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/gene.gtf"), sep = ""))
...
# Run stringtie on haplotype-specific BAM files for PMD-P11 
# Use "-E 5" during StringTie run because cryptic donor splice site is being "overcorrected" back to canonical donor splice site
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_1"), showWarnings = FALSE)
dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_2"), showWarnings = FALSE)
system(paste(stringtie, file.path(workdir, "PMD", sample.map["PMD-P11"], "RNA/stripe/target_genes/SUCLG1/hap2_reads.bam"), "-G", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/gene.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_2/output.gtf"), "-L -s 5 -c 5 -u -M 0 -E 5"))

# Merge output.gtf with gene.gtf via gffcompare
system(paste(gffcompare, "-M -D -T -C -X", "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_2/output.gtf"), file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/gene.gtf")))
...
if (FALSE) {
    "/scr1/users/wangr5/tools/SQANTI3-5.2.2/sqanti3_qc.py" --force_id_ignore --skipORF -o sqanti3 \
        -d "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp" --report skip \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.combined.gtf" \
        "/mnt/isilon/lin_lab_share/STRIPE/manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/gene.gtf" \
        "/scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa"
}

system(paste("python /scr1/users/wangr5/tools/Annotate_ORF.py -i", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.combined.gtf"), 
    "-a", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/gene.gtf"), "-f /scr1/users/wangr5/references/GRCh38.primary_assembly.genome.fa", 
    "-o", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.updated.gtf")))

# Re-run stringtie on TEQUILA-seq BAM files
system(paste(stringtie, file.path(workdir, "PMD", sample.map["PMD-P11"], "RNA/stripe/target_genes/SUCLG1/hap1_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_1/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
system(paste(stringtie, file.path(workdir, "PMD", sample.map["PMD-P11"], "RNA/stripe/target_genes/SUCLG1/hap2_reads.bam"), 
    "-G", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
    "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/Haplotype_2/output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))

# Run stringtie on TEQUILA-seq BAM files for other cohort individuals
cohort.samples <- read.table(file.path(workdir, "PMD/samples.txt"), sep = "\t", header = TRUE) %>% filter((Provider == "Rebecca Ganetzky" & ID != sample.map["PMD-P11"]) | 
    (Provider == "Marni Falk" & Status != "Diagnosed" & !grepl("-MF", ID))) %>% pull(ID)
for (sample.id in cohort.samples) {
    dir.create(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", sample.id), showWarnings = FALSE)
    system(paste(samtools, "view -hb -F 256 -q 1", file.path(workdir, "PMD", sample.id, "RNA", paste(sample.id, "TEQUILA.bam", sep = "_")), 
        paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""), " > ", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", 
        sample.id, "input.bam")))
    system(paste(samtools, "index", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", sample.id, "input.bam")))
    system(paste(stringtie, file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", sample.id, "input.bam"), "-G", 
        file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.combined.gtf"), "-o", file.path(workdir, 
        "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", sample.id, "output.gtf"), "-s 5 -c 5 -u -M 0 -e -B -L"))
}
...
for (sample.id in c("Haplotype_1", "Haplotype_2", cohort.samples)) {
    outDF <- full_join(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp", sample.id, 
...
sqanti3 <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/sqanti3_classification.txt"), 
...
# Only keep transcripts featuring usage of a cryptic acceptor splice site in SUCLG1 intron 1 with the highest isoform-level proportion in haplotype 2 or 
# transcripts with an average isoform-level proportion ≥ 10% across other cohort individuals
keepTranscripts <- c("ENST00000393868.7", "TCONS_00000002")
...
gtfDF <- read.table(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp/merged.updated.gtf"), sep = "\t", header = FALSE, comment = "#") %>%
...
p1 <- ggplot() + geom_rect(data = utrDF, fill = "white", xmin = 1-utrDF$V4, xmax = 1-utrDF$V5, ymin = utrDF$Transcript_Number - 0.25,
    ...
    panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") + scale_fill_manual(values = rev(c("#388BBF", "#F9AB61")))

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/Figure_S10c.pdf"), plot = p1, width = 3, height = 0.75)
...
p2 <- ggplot() + geom_bar(data = summaryDF, aes(x = Group, y = Prop_Mean * 100, fill = Transcript_ID, group = Transcript_ID), linewidth = 0.5,
    ...
    jitter.height = 0, dodge.width = 0.75)) + theme_classic() + ylab("SUCLG1 isoform proportion (%)") + theme(panel.grid.major = element_blank(), 
    ...
    scale_fill_manual(values = c("#388BBF", "#F9AB61", "#BEBEBE")) + scale_color_manual(values = rep("black", 5)) + 
    ylim(0, 100) + guides(fill = guide_legend(nrow = 1))

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/Figure_S10d.pdf"), plot = p2, width = 3, height = 2.25)
...
p3 <- ggplot() + geom_bar(data = hapDF, aes(x = Count, y = Group, fill = Transcript_ID), stat = "identity", linewidth = 0.5, position = "stack", width = 0.75) +
    ...
    legend.key.size = unit(0.3, "cm"), legend.title = element_blank(), legend.text = element_text(color = "black", size = 6)) + 
    scale_fill_manual(values = c("#3188BD", "#FCAE62", "#BEBEBE")) + xlim(0, 3000)

ggsave(file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/Figure_S10e.pdf"), plot = p3, width = 3, height = 1.5)

# Remove intermediate files
system(paste("rm -rf", file.path(workdir, "manuscript/Revisions/20250228/Supplementary_Figures/Figure_S10/tmp")))
