...
# (d) Usage frequencies for a splice junction involving skipping of PMM2 exons 3 to 7 across cohort individuals and 
...
# Used the following Python code to retrieve read support and total coverage for splice junction chr16:8801911-8847723
# in cohort samples
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_2/tmp')
    sampleDF = pd.read_csv(workdir + '/CDG/samples.txt', sep = '\t', header = 0)
    sampleDF = sampleDF[(sampleDF['Provider'] != 'Lan Lin') & ~sampleDF['ID'].isin({'E1877', 'CDG-152-1'})]
    targetDF = pd.read_csv(workdir + '/CDG/references/target_genes.bed', sep = '\t', header = None)
    gene_id = targetDF[targetDF[4] == 'PMM2'].values[0][3]
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_2/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_2/tmp/input.gtf', sep = '\t', header = None)
    ...
    for sampid in sampleDF['ID']:
        ...
        junctions = junctions[(junctions['Start'] == '8801911') | (junctions['End'] == '8847723')].merge(
            pd.DataFrame([['chr16', '8801911', '8847723']], columns = ['Chrom', 'Start', 'End']), how = 'outer').fillna(0)
        junctions['PSI'] = junctions['Read_Counts']/(np.nan if junctions['Read_Counts'].sum() < 20 else junctions['Read_Counts'].sum())
        output[sampid] = junctions[(junctions['Start'] == '8801911') & (junctions['End'] == '8847723')]['PSI'].item()

    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/CDG-152-1/PMM2/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'PMM2'].values[0][0],
            targetDF[targetDF[4] == 'PMM2'].values[0][1], targetDF[targetDF[4] == 'PMM2'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
        ...
        junctions = junctions[(junctions['Start'] == '8801911') | (junctions['End'] == '8847723')].merge(
            pd.DataFrame([['chr16', '8801911', '8847723']], columns = ['Chrom', 'Start', 'End']), how = 'outer').fillna(0)
        junctions['PSI'] = junctions['Read_Counts']/(np.nan if junctions['Read_Counts'].sum() < 20 else junctions['Read_Counts'].sum())
        output['Haplotype_' + str(idx)] = junctions[(junctions['Start'] == '8801911') & (junctions['End'] == '8847723')]['PSI'].item()

    pd.DataFrame(output.items(), columns = ['Sample_ID', 'PSI']).to_csv(workdir + '/manuscript/Revisions/20241219/Main_Figures/Figure_2/tmp/cohort.tsv', 
        sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_2/Figure_2d.pdf")

# Retrieve read support and total coverage for splice junction chr16:8801911-8847723 in GTEx controls
gtex.splice <- full_join(read.table(file.path(workdir, "CDG/references/GTEx_v8/Fibroblast/target_genes.junction_counts.txt"), 
    ...
    filter(Chrom == "chr16" & (Start == "8801911" | End == "8847723")) %>% distinct(), tibble(Chrom = "chr16", 
    Start = "8801911", End = "8847723"), by = join_by(Chrom, Start, End)) %>% replace(is.na(.), 0)
junction.counts <- as.numeric(gtex.splice %>% filter(Chrom == "chr16" & Start == "8801911" & End == "8847723") %>% 
...
# Retrieve read support and total coverage for splice junction chr16:8801911-8847723 in cohort samples
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_2/tmp/cohort.tsv"), sep = "\t", 
...
p <- ggplot() + geom_bar(data = summaryDF, aes(x = PSI * 100, y = Group, fill = Group), stat = "identity", width = 0.75) + geom_point(data = outDF,
    ...
    height = 0.2)) + theme_classic() + xlab("PMM2 exons 3-7 skipping (%)") + theme(panel.grid.major = element_blank(), 
