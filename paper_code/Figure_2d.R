...
# (d) Usage frequencies for a splice junction involving skipping of PMM2 exons 3 to 7 in individual CDG-P12 (resolved by 
...
if (FALSE) {
    ...
    _ = os.system('mkdir -p ' + workdir + '/manuscript/Revisions/20250228/Main_Figures/Figure_2/tmp')
    sampleDF = pd.read_csv(workdir + '/CDG/samples.txt', sep = '\t', header = 0)
    sampleDF = sampleDF[(sampleDF['Provider'] != 'Lan Lin') & ~sampleDF['ID'].isin({sampleMap['PMD-C01'], sampleMap['CDG-P12']})]
    targetDF = pd.read_csv(workdir + '/CDG/references/target_genes.bed', sep = '\t', header = None)
    gene_id = targetDF[targetDF[4] == 'PMM2'].values[0][3]
    _ = os.system('grep ' + gene_id + ' ' + gencode + ' > ' + workdir + '/manuscript/Revisions/20250228/Main_Figures/Figure_2/tmp/input.gtf')
    geneDF = pd.read_csv(workdir + '/manuscript/Revisions/20250228/Main_Figures/Figure_2/tmp/input.gtf', sep = '\t', header = None)
    ...
    for idx in [1, 2]:
        junctions = dict()
        infile = workdir + '/manuscript/Revisions/20250228/Supplementary_Tables/Table_S6/' + sampleMap['CDG-P12'] + '/PMM2/hap' + str(idx) + '_reads.bam'
        for read in pysam.AlignmentFile(infile, 'rb').fetch(targetDF[targetDF[4] == 'PMM2'].values[0][0],
            targetDF[targetDF[4] == 'PMM2'].values[0][1], targetDF[targetDF[4] == 'PMM2'].values[0][2]):
            for item in ParseCigarJunctions(read.cigarstring, read.reference_name, read.reference_start):
                if any([int(ss) in sites for ss in item.split('_')[1:]]):
    ...
    pd.DataFrame(output.items(), columns = ['Sample_ID', 'PSI']).to_csv(workdir + '/manuscript/Revisions/20250228/Main_Figures/Figure_2/tmp/cohort.tsv', 
        sep = '\t', index = False)
}

# Establish working directories and file paths
workdir <- "/mnt/isilon/lin_lab_share/STRIPE"
outfile <- file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_2/Figure_2d.pdf")
...
outDF <- bind_rows(outDF, read.table(file.path(workdir, "manuscript/Revisions/20250228/Main_Figures/Figure_2/tmp/cohort.tsv"), sep = "\t", 
