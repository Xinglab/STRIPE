...
# Haplotype-resolved read coverage over FH in patient FH-2209, who is heterozygous for a frameshift duplication 
# (NM_000143.4:c.40dup, p.Leu14ProfsTer42). 
...
target.gene <- read.table(file.path(workdir, "/PMD/references/target_genes.bed"), sep = "\t", header = FALSE) %>% filter(V5 == "FH")
...
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Figures/Figure_S6/Figure_S6b.pdf")
...
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/FH-2209/FH/hap1_reads.bam"),
    ">", file.path(dirname(outfile), "tmp/hap1_coverage.txt")))
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/FH-2209/FH/hap2_reads.bam"),
...
gene.coverage <- ggplot(plotDF, aes(x = 1-Coord, y = Coverage, color = Group)) + 
    ...
    ylab("Read coverage") + scale_color_manual(values = c("#EC7B02", "#90AFB1")) + ylim(0, 4000) + 
...
gene.structure <- ggplot() + geom_rect(data = utrDF, fill = "white", xmin = 0.75, xmax = 1.25, ymin = 1-utrDF$V4, ymax = 1-utrDF$V5, 
    ...
    axis.title.x = element_text(color = "black", size = 7)) + coord_flip() + ylab("FH (NM_000143.4)") 
