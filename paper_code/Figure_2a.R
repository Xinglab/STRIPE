...
# Haplotype-resolved read coverage over TRMU in patient Q1642, who is heterozygous for a 1.8-kb structural deletion that 
...
outfile <- file.path(workdir, "manuscript/Revisions/20241219/Main_Figures/Figure_2/Figure_2a.pdf")
...
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/Q1642/TRMU/hap1_reads.bam"),
    ">", file.path(dirname(outfile), "tmp/hap1_coverage.txt")))
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "manuscript/Revisions/20241219/Supplementary_Tables/Table_S6/Q1642/TRMU/hap2_reads.bam"),
...
# hap2DF is empty (no reads assigned to haplotype 2) so draw segment from (0, 0) to (1, 0)
plotDF <- bind_rows(tibble(Coord = c(head(hap1DF$Start, 1), rep(tail(hap1DF$Start, -1), each = 2), tail(hap1DF$End, 1)), 
    Coverage = rep(hap1DF$Coverage, each = 2)) %>% mutate(Group = "Haplotype 1"), tibble(Coord = c(0, 1), 
    Coverage = c(0, 0)) %>% mutate(Group = "Haplotype 2")) %>%
    mutate(Group = factor(Group, levels = c("Haplotype 1", "Haplotype 2")))
gene.coverage <- ggplot(plotDF, aes(x = Coord, y = Coverage, color = Group)) + 
    ...
    ylab("Read coverage") + scale_color_manual(values = c("#EC7B02", "#90AFB1")) + ylim(0, 1500) + 
