...
# (a) Haplotype-resolved expression of NGLY1 in individual FCDGC-02003, who is heterozygous for a rare frameshift deletion 
# variant (NM_018297.4: c.1533_1536del, p.Asn511LysfsTer51). (b) Minor haplotype expression ratios for NGLY1 across cohort 
# samples and tissue-matched GTEx controls.
...
outfile <- file.path(workdir, "manuscript/Supplementary_Figures/Figure_S13/Figure_S13.pdf")

# =====================================================================================================================
#                                                      PANEL A
# =====================================================================================================================

# Create a temporary directory in the folder for Supplementary Figure 13
...
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "CDG/FCDGC-02003/RNA/stripe/target_genes/NGLY1/hap1_reads.bam"), ">",
    file.path(dirname(outfile), "tmp/hap1_coverage.txt")))
system(paste(samtools, "depth -r", paste(target.gene$V1, ":", target.gene$V2+1, "-", target.gene$V3, sep = ""),
    file.path(workdir, "CDG/FCDGC-02003/RNA/stripe/target_genes/NGLY1/hap2_reads.bam"), ">",
...
gene.coverage <- ggplot(bind_rows(tibble(Coord = c(head(hap1DF$Start, 1), rep(tail(hap1DF$Start, -1), each = 2), tail(hap1DF$End, 1)), 
    Coverage = rep(hap1DF$Coverage, each = 2)) %>% mutate(Group = "FCDGC-02003 (haplotype 1)"), tibble(Coord = c(head(hap2DF$Start, 1), 
    rep(tail(hap2DF$Start, -1), each = 2), tail(hap2DF$End, 1)), Coverage = rep(hap2DF$Coverage, each = 2)) %>% mutate(Group = "FCDGC-02003 (haplotype 2)")) %>%
    mutate(Group = factor(Group, levels = c("FCDGC-02003 (haplotype 1)", "FCDGC-02003 (haplotype 2)"))), aes(x = 1 - Coord, y = Coverage, color = Group)) + 
...
gene.structure <- ggplot() + geom_rect(data = utrDF, fill = "white", xmin = 0.75, xmax = 1.25, ymin = 1 - utrDF$V4, ymax = 1 - utrDF$V5, 
    ...
    axis.title.x = element_text(color = "black", size = 7)) + coord_flip() + ylab("NGLY1 (NM_018297.4)") 
...
outDF <- tibble(Minor = minor.counts, Major = major.counts) %>% mutate(Group = "Fibroblasts\n(GTEx v8)", Label = "")
...
for (sampid in cohort.samples) {
    infile <- file.path(workdir, "CDG", sampid, "RNA/stripe/target_genes/NGLY1/phasing_stats.txt")
...
outDF <- bind_rows(outDF, tibble(Minor = minor.counts, Major = major.counts, Group = "Fibroblasts\n(cohort)", Label = sample.labels)) %>% 
    mutate(Group = factor(as.character(Group), levels = c("Fibroblasts\n(GTEx v8)", "Fibroblasts\n(cohort)"))) %>% filter(Minor + Major >= 20) %>%
    mutate(Ratio = Minor/(Minor+Major))

p2 <- ggplot() + geom_jitter(data = outDF %>% filter(Label == ""), aes(x = Group, y = Ratio * 100, color = Group), stroke = NA, alpha = 0.5, height = 0, 
    width = 0.2) + geom_point(data = outDF %>% filter(Label != ""), aes(x = Group, y = Ratio * 100), stroke = NA, color = "#E43321") + theme_bw() + 
    ylim(0, 50) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), 
    ...
    linewidth = 0.25), legend.position = "none", plot.title = element_text(color = "black", size = 7, hjust = 0.5, face = "italic"), 
    axis.title.x = element_blank()) + ylab("Minor haplotype\nexpression ratio (%)") + scale_color_manual(values = c("#1A78AC", "#6FC7CE")) + ggtitle("NGLY1")

# Assemble p1 and p2 onto the same plotting grid
p <- plot_grid(p1, p2, nrow = 1, rel_widths = c(2.25, 1), labels = c("a", "b"), label_size = 8)
ggsave(outfile, plot = p, width = 6.5, height = 1.75)
