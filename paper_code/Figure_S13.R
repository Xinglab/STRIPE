...
# variant (NM_018297.4: c.1533_1536del, p.Asn511LysfsTer51). (b) Major and minor haplotype read counts for NGLY1 across 
...
gene.coverage <- ggplot(bind_rows(tibble(Coord = c(head(hap1DF$Start, 1), rep(tail(hap1DF$Start, -1), each = 2), tail(hap1DF$End, 1)), 
    ...
    legend.text = element_text(color = "black", size = 6)) + ylab("TEQUILA-seq read coverage") + scale_color_manual(values = c("#59AAD1", "#DF5C79")) + ylim(0, 6000)
...
outDF <- bind_rows(outDF, tibble(Minor = minor.counts, Major = major.counts, Group = sample.labels)) %>% mutate(Group = factor(as.character(Group), 
    levels = c("Fibroblasts (GTEx)", "Fibroblasts (cohort)", "FCDGC-02003")))
...
p2 <- ggplot(outDF, aes(x = Major, y = Minor, color = Group)) + geom_point(stroke = NA, alpha = 0.8) + theme_bw() + 
    ...
    xlab("Major haplotype read count") + scale_color_manual(values = c("#1A78AC", "#6FC7CE", "#E43321")) + ggtitle("NGLY1")
