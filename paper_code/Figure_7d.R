...
outfile <- file.path(workdir, "manuscript/Main_Figures/Figure_7/Figure_7d.pdf")

# Pull out all reads supporting ALG1 and its pseudogenes
dir.create(file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp"), showWarnings = FALSE)
...
for (i in 1:length(gene.list)) {
    ...
    system(paste("grep", names(gene.list[i]), gencode, ">", file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/input.gtf")))
    gene.coord <- read.table(file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/input.gtf"), sep = "\t",
    ...
    system(paste(samtools, "view -hb", file.path(workdir, "CDG/CDG-0367/RNA/CDG-0367_TEQUILA.bam"), gene.coord,
        ">", file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/input.bam")))
    system(paste(samtools, "flagstat -O tsv", file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/input.bam"), ">",
        file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/flagstat.txt")))
    ...
    flagDF <- read.table(file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/flagstat.txt"), sep = "\t", header = FALSE)
...
system(paste("rm", file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp/*")))
system(paste("rmdir", file.path(workdir, "manuscript/Main_Figures/Figure_7/tmp")))
